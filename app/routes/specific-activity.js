    ...
    model(param) {
        ...
        this.get('session').authorize('authorizer:oauth2', (headerName, headerValue) => {
            ...
            if(this.get('session.data.authenticated.token_data.college_role') === "god") {
                ...
                //ERROR CHECK console.log("Admin! -- " + this.get('session.data.authenticated.token_data.college_role'));
            };

            // Set the logged in user to be leader if they have admin priviledges
            if (model.adminPriv) {
                //ERROR CHECK console.log("Admin! part.2");
                model.leading = true;
            };

            //ERROR CHECK console.log(this.get('session.data.authenticated'));
